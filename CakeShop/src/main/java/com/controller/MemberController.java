...
public class MemberController {
	...
	public ModelAndView QueryBySMID(HttpServletRequest ques, ModelMap model, @ModelAttribute("Member") Member stu) {		
		String str = ques.getSession().getAttribute("memberId").toString();
		if(str!=null) {
		...
		model.addAttribute("password", stu.getPassword());
		model.addAttribute("memberName", stu.getMemberName());
		model.addAttribute("gender", stu.getGender()); 
		model.addAttribute("birthday", stu.getBirthday()); 
		...
		@RequestMapping(value = "/updateFM.do", method = RequestMethod.POST, params = { "memId", "memName", "memPwd",
				"memGender", "memBirthday", "memEmail", "memAddress", "memPhoneNumber"})
		public ModelAndView updateFM(@RequestParam("memId") Integer memId, @RequestParam("memName") String memName,
				@RequestParam("memPwd") String memPwd, @RequestParam("memGender") String memGender,
				@RequestParam("memBirthday") String memBirthday, @RequestParam("memEmail") String memEmail,
				@RequestParam("memAddress") String memAddress, @RequestParam("memPhoneNumber") String memPhoneNumber,
				Member member, Model model, HttpServletResponse resp) {
...
//			String password=m1.getPassword();
			...
			if (memName.equals(""))
				memName = m1.getMemberName();
			if (memGender.equals(""))
				memGender = m1.getGender();
			if (memBirthday.equals(""))
				memBirthday = m1.getBirthday();
			if (memEmail.equals(""))
			...
			member = new Member(memId, account, memPwd, memName, memGender, memBirthday, memEmail, memAddress,
					memPhoneNumber);
			dao.updateObj(member);
			System.out.println("Member name: " + m1.getMemberName() + " has amended his/her info.");
			model.addAttribute("member", member);		
			return new ModelAndView("redirect:/quySMID.do");
			
		}
	
	@RequestMapping(value = "/memlogout.do", method = RequestMethod.POST)
	protected void Logout(HttpServletRequest ques, HttpServletResponse resp) throws ServletException, IOException {
		String str = ques.getSession().getAttribute("memberId").toString() ;		
		System.out.println("MemberId : " + str + " has logged out");
